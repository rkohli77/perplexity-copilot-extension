  ...
  (globalThis as any).fetch
    ...
    : (fetchFn ??
      (fetchFn = async (...args: any[]) => {
        ...
        const fetchFnAny = (mod.default ?? mod) as (...args: any[]) => Promise<any>;
...
class PerplexityChatProvider implements vscode.WebviewViewProvider {
  ...
  constructor(
    ...
    try {
      ...
      const configKey = vscode.workspace.getConfiguration("perplexity").get<string>("apiKey");
  ...
  public isSidebarLoaded(): boolean {
  ...
  private async askPerplexity(prompt: string): Promise<string> {
    ...
    if (!this._apiKey) {
      vscode.window.showErrorMessage(
        "Perplexity: missing API key. Set `perplexity.apiKey` in Settings, SecretStorage, or PERPLEXITY_API_KEY in environment."
      );
    ...
    const payload = {
      model: vscode.workspace.getConfiguration("perplexity").get<string>("model") || "sonar",
      messages: [{ role: "user", content: prompt }],
    };
    const fetch: any = getFetch();
    const res: any = await fetch("https://api.perplexity.ai/chat/completions", {
      method: "POST",
      headers: {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    context.subscriptions.push(
      vscode.window.registerWebviewViewProvider("perplexityChat", provider, {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("perplexity.showSidebar", async () => {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("perplexity.ask", async () => {
    ...
    // Command: show config
    context.subscriptions.push(
      vscode.commands.registerCommand("perplexity.showConfig", async () => {
        const cfgKey = vscode.workspace.getConfiguration("perplexity").get<string>("apiKey") || "";
        const envKey = process.env.PERPLEXITY_API_KEY || "";
        const secretKey = await context.secrets.get("perplexity.apiKey") || "";
        const cfgModel = vscode.workspace.getConfiguration("perplexity").get<string>("model") || "";
    ...
    // Command: set API key
    context.subscriptions.push(
      vscode.commands.registerCommand("perplexity.setApiKey", async () => {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand("perplexity.openChatPanel", async () => {
    ...
    // Command: pick model
    context.subscriptions.push(
      vscode.commands.registerCommand("perplexity.pickModel", async () => {
        const cacheKey = "perplexity.models.cache";
        const cacheTtl = 1000 * 60 * 60 * 24;
        let cached = context.globalState.get<{ timestamp: number; models: Array<{ id: string; desc?: string }> }>(cacheKey);
        const now = Date.now();
        if (!cached || now - cached.timestamp > cacheTtl || !Array.isArray(cached.models) || !cached.models.length) {
          try {
            ...
            if (res.ok) {
              ...
              let modelsRaw: any[] = [];
              if (Array.isArray(body)) modelsRaw = body;
              else if (Array.isArray(body?.models)) modelsRaw = body.models;
              const models = modelsRaw.map(m => {
                if (typeof m === "string") return { id: m, desc: "" };
...
class PerplexityChatPanel {
  public static currentPanel: PerplexityChatPanel | undefined;
  private readonly _panel: vscode.WebviewPanel;
  private readonly _provider: PerplexityChatProvider;

  public static createOrShow(extensionUri: vscode.Uri, provider: PerplexityChatProvider, initialPrompt?: string) {
  ...
  private constructor(panel: vscode.WebviewPanel, provider: PerplexityChatProvider, initialPrompt?: string) {
    this._panel = panel;
  ...
  private sendInitialPrompt(prompt: string) {
